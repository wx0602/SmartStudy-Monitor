...
class PostureDetector:
    ...
    def process_frame(self, image):
        ...
        output_data = {
            # 评估结果（判断部分）
            "is_shoulder_tilted": False,        # A.肩膀是否倾斜
            "is_head_forward": False,           # B.头部是否前伸
            "is_hunchback": False,              # C.是否驼背
            "is_neck_tilted": False,            # D.颈部是否侧倾
            "dist_screen": "normal",            # E.距离屏幕: too_close/normal
            "body_lean": "centered",            # F.躯干位置
            
            # 各项评分和角度（数值部分）
            "shoulder_tilt_angle": 0.0,         # A.肩膀倾斜角度
            "head_forward_degree": 0.0,         # B.头部前伸程度
            "hunchback_degree": 0.0,            # C.驼背程度
            "neck_tilt": 0.0,                   # D.颈部侧倾角度
            "shoulder_screen_ratio": 0.0,       # E.肩膀宽度与屏幕宽度比
            "lean_degree":0,                    # F.躯干偏移程度
            "stability_score": 100,             # G.稳定性评分
        ...
        if results.pose_landmarks:
            ...
            # === A. 计算肩膀倾斜 ===
            # 逻辑：计算两肩连线的斜率或角度
            # 返回的内容包括 shoulder_tilt_angle (float) 和 is_shoulder_tilted (bool)
            ...
            # === B. 判断头部前伸 ===
            # 逻辑：比较鼻子Z和肩膀中点Z，Z越小越近。鼻子比肩膀越近，head_forward_degree 越大，前伸越严重。
            # 归一化：除以肩膀宽度，防止离远离近影响数值
            # 返回的内容包括 is_head_forward (bool) 和 前伸越严重 (float)
            if shoulder_width > 0:
            ...
            # === C. 判断驼背 ===
            # 逻辑：计算脖子视觉长度（耳朵Y到肩膀Y的距离），如果长度变短，就是驼背
            # 返回的内容包括 is_hunchback (bool)和 hunchback_degree (float)
            if shoulder_width > 0:
            ...
            # === D. 计算颈部侧倾 ===
            # 逻辑：根据两耳连线计算侧倾角度
            # 返回的内容包括 neck_tilt (float) 和 is_neck_tilted (bool)
            ...
            # 返回的内容包括 dist_screen (str) 和 shoulder_screen_ratio (float)
            ...
            # 归一化偏差： (中点X - 画面中心) / 画面宽
            # 返回的内容包括 body_lean (str) 和 lean_degree(float)
            ...
            # === G. 稳定性分析 ===
            # 逻辑：记录肩膀中点位置的历史，计算抖动程度，抖动越大，稳定性评分越低
            # 返回的内容包括 stability_score (int)
            ...
            if len(self.history) > 5:
                ...
                std_devs = np.std(history_array, axis=0) # 得到 [std_x, std_y]
                # 归一化：将像素抖动转为相对于画面宽度的比例（乘以1000方便观察）
